    ...
    <section className="w-full bg-white py-8 md:py-16 border-t border-gray-100">
      <div className="max-w-7xl mx-auto px-6 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {newsList.map((news) => (
            <Link to={`/frett/${news.slug}`} key={news.slug} aria-label={`Lesa grein: ${news.title}`} className="group flex flex-col bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-[#c8102e]/30">
